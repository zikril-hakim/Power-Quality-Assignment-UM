...
for i = 1:height(nfault_bus)
    if nfault_bus{i,3} < 0.4
        fault5no = fault5no + nfault_bus{i,5};
    end
end
...
for i = 1:height(nfault_bus)
    if nfault_bus{i,4} < 0.4
        fault14no = fault14no + nfault_bus{i,5};
    end
end
...
disp('Question 3')
disp(' ')
fprintf('Bus 5    :     %.2f  fault occurence\n', fault5no)
fprintf('Bus 14   :     %.2f  fault occurence\n', fault14no)
